    ...
    public function render_scheduler_slot_manager(scheduler_slot_manager $slotman) {
        ...
        foreach ($slotman->slots as $slot) {
            ...
            if ($slot->editable && $slot->isappointed) {
				...
				$confirmrevoke = new confirm_action('Do you want to revoke ?');
                $actions .= $this->action_icon($url, new pix_icon('s/no', get_string('revoke', 'scheduler')), $confirmrevoke);
            }
			
			if ($slot->editable && $slot->isappointed) {
                $url = new moodle_url($slotman->actionurl, array('what' => 'confirmslot', 'slotid' => $slot->slotid));
                $confirmslot = new confirm_action('Do you want to Confirm this Slot ?');
                $actions .= $this->action_icon($url, new pix_icon('t/approve', 'Do you want to Confirm this Slot'), $confirmslot);
            }
