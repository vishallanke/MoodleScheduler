...
//scheduler_messenger::send_slot_notification($slot, 'bookingnotification', 'teachercancelled', $teacher, $student, $teacher, $student, $COURSE);
                scheduler_messenger::send_slot_notification_cancelled($slot, 'bookingnotification', 'teachercancelled', $teacher, $student, $teacher, $student, $COURSE, $txt);
	...
	case 'confirmslot':
       ...
       // if ($scheduler->allownotifications) {
            foreach ($oldstudents as $oldstudent) {
				...
				$teacher1 = $DB->get_record('user', array('id' => 1391));
                $teacher2 = $DB->get_record('user', array('id' => 947)); 
                $teacher3 = $DB->get_record('user', array('id' => 1347)); 
                $teacher4 = $DB->get_record('user', array('id' => 32)); 

                scheduler_messenger::send_slot_notification_confirmed($slot, 'bookingnotification', 'teachercancelled',
                                        $teacher, $student, $teacher, $student, $COURSE, $teacher1, $teacher2, $teacher3, $teacher4);
										
														// fetch all trainers
				//$teacher1 = $DB->get_record('user', array('id' => 32));
				//scheduler_messenger::send_slot_notification_confirmed($slot, 'bookingnotification', 'teachercancelled', $teacher1, $teacher1, $teacher1, $student, $COURSE);					

					// Chennai Send Email to Others			
